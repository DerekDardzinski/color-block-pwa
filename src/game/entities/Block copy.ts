...
//     const extrusionDepth = 10; // Depth of 3D extrusion effect in pixels

//     // Draw layers in order (back to front)
//     // this.drawDropShadow();
//     this.drawExtrusionLayers(extrusionDepth);
...
//     const cornerRadius = this.cellSize * 0.12;
...
//     this.drawRoundedPath(points, cornerRadius, shadowOffset, shadowOffset);
//     this.graphics.fillPath();
//   }

//   /**
//    * Draw extrusion layers to create 3D depth effect
//    * Offsets only North and West edges inward to create visible south/east extrusion
//    */
//   private drawExtrusionLayers(depth: number): void {
//     const { points, edgeDirections } = this.buildOuterEdgePath();
//     const cornerRadius = this.cellSize * 0.12;
//     const baseColor = this.getColorValue();
//     const darkerColor = this.getDarkerColor(baseColor);

//     // Bottom layer: Offset North and West edges inward by depth/2
//     // This reveals the darker extrusion on South and East edges
//     const bottomPath = this.offsetPathSelective(points, edgeDirections, ['N', 'W'], depth / 2);

//     this.graphics.fillStyle(darkerColor, 0.85);
//     this.drawRoundedPath(bottomPath, cornerRadius);
//     this.graphics.fillPath();
//   }
...
//   /**
//    * Selectively offset specific edges of a path inward
//    * Only offsets edges matching the specified directions
//    */
//   private offsetPathSelective(
//     points: { x: number; y: number }[],
//     edgeDirections: ('N' | 'E' | 'S' | 'W')[],
//     targetDirections: ('N' | 'E' | 'S' | 'W')[],
//     offset: number
//   ): { x: number; y: number }[] {
//     if (offset === 0 || points.length < 3) return points;

//     const offsetPath: { x: number; y: number }[] = [];
//     const targetSet = new Set(targetDirections);
...
//       // Get the directions of edges before and after this point
//       const prevEdgeDir = edgeDirections[i === 0 ? edgeDirections.length - 2 : i - 1];
//       const currEdgeDir = edgeDirections[i];

//       // Check if each edge should be offset
//       const offsetPrevEdge = targetSet.has(prevEdgeDir);
//       const offsetCurrEdge = targetSet.has(currEdgeDir);
...
//       // Only include normals from edges that should be offset
//       let offsetX = 0;
//       let offsetY = 0;
//       let count = 0;
...
//           const firstNormal = offsetPrevEdge ? normal1 : normal2;
//           const dotProduct = firstNormal.x * offsetX + firstNormal.y * offsetY;
//           const offsetDistance = dotProduct > 0.001 ? offset / dotProduct : offset;

//           const newX = curr.x + offsetX * offsetDistance;
//           const newY = curr.y + offsetY * offsetDistance;
//           offsetPath.push({ x: newX, y: newY });
//         } else {
//           offsetPath.push({ x: curr.x, y: curr.y });
//         }
//       } else {
//         // Neither adjacent edge should be offset, keep original position
//         offsetPath.push({ x: curr.x, y: curr.y });
//       }
//     }
...
//    * Returns path points and edge direction metadata for selective offsetting
//    */
//   private buildOuterEdgePath(): {
//     points: { x: number; y: number }[];
//     edgeDirections: ('N' | 'E' | 'S' | 'W')[];
//   } {
//     const cellSize = this.cellSize;
...
//     const edgeDirections: ('N' | 'E' | 'S' | 'W')[] = [];
//     const visited = new Set<number>();

//     // Start with the first edge
//     if (edges.length === 0) return { points: [], edgeDirections: [] };
...
//       // Store the direction of the current edge for this segment
//       edgeDirections.push(currentEdge.direction);
...
//     // Push the direction of the last edge (which closes back to the start)
//     if (visited.size > 0) {
//       edgeDirections.push(currentEdge.direction);
//     }
...
//       // Add a sentinel direction for the closing segment (wraparound logic)
//       edgeDirections.push(edgeDirections[0]);
//     }

//     return { points, edgeDirections };
//   }

//   /**
//    * Draw the shape as a continuous rounded polygon
//    * Offsets only South and East edges inward to align with extrusion layer
//    */
//   private drawContinuousShape(color: number, _cornerRadius: number): void {
//     const cornerRadius = this.cellSize * 0.12;
//     const extrusionDepth = 10; // Match the depth from renderBlock

//     // Build the outer edge path
//     const { points, edgeDirections } = this.buildOuterEdgePath();

//     // Top layer: Offset South and East edges inward by depth/2
//     // This allows the bottom layer to show through on these edges
//     const topPath = this.offsetPathSelective(points, edgeDirections, ['S', 'E'], extrusionDepth / 2);
...
//     this.drawRoundedPath(topPath, cornerRadius);
//     this.graphics.fillPath();

//     // Add outer glow for "lit from above" effect
//     this.graphics.lineStyle(2, highlightColor, 0.4);
//     this.drawRoundedPath(topPath, cornerRadius);
//     this.graphics.strokePath();

//     // Add inner edge highlight for enhanced 3D depth
//     this.graphics.lineStyle(1, lightColor, 0.3);
//     this.drawRoundedPath(topPath, cornerRadius);
//     this.graphics.strokePath();
//   }
